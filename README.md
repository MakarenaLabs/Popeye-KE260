...
This project aims to port the Popeye cabinet emulator to the Xilinx KR260 FPGA. The Popeye emulator allows you to play Popeye cabinet games on FPGA hardware.
...
- `tools`: contains scripts for converting a Popeye games file to COE format for loading the ROM into the EPROM(s) IPs.

## Loading a New Cartridge statically

To load an Popeye cartridge statically, generate the COE file and load it via Vivado interface. Follow these steps:
...
* Original FPGA conversion 
https://github.com/MiSTer-devel/Arcade-Popeye_MiSTer